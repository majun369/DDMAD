...
def sliding_local_infoNCE(z1, z2, pooling='max', temperature=1.0, k=16, sliding=16, negtive_number=16):
    ...
    anchors_array_pooling = anchors_array_pooling.reshape(
        anchors_array_pooling.shape[0] * anchors_array_pooling.shape[1], 1, anchors_array_pooling.shape[2])
    poss_array_pooling = poss_array_pooling.reshape(poss_array_pooling.shape[0] * poss_array_pooling.shape[1], 1,
                                                    poss_array_pooling.shape[2])
    negs_array_pooling = negs_array_pooling.reshape(negs_array_pooling.shape[0] * negs_array_pooling.shape[1],
                                                    negtive_number, negs_array_pooling.shape[-1])
...
def local_infoNCE(z1, z2, pooling='max', temperature=1.0, k=16):
    ...
    pos_labels = labels.cuda(device=z1.get_device())
    pos_labels[k - 1, k - 2] = 1.0

    neg_labels = labels.T + labels + torch.eye(k)
    neg_labels[0, 2] = 1.0
    neg_labels[-1, -3] = 1.0
    neg_labels = neg_labels.cuda(device=z1.get_device())
...
def InfoNCE(z1, z2, temperature=1.0, l1out=False):
    ...
    mask = torch.eye(z1.shape[0], dtype=torch.bool).cuda(device=z1.get_device())
